...
class CommentPublisher {
    ...
    public static init(): void {
        document.addEventListener('click', (event) => {
            ...
            const button = target.closest('.comment-publish-button') as CommentElement;
    ...
    private static async handlePublishClick(button: CommentElement): Promise<void> {
        const commentId = button.dataset.id;
        
        try {
            await this.togglePublishStatus(commentId);
    ...
    private static async togglePublishStatus(commentId: string): Promise<void> {
        const formData = new URLSearchParams();
        formData.append('id', commentId);

        const response = await fetch('/admin/comments/publish', {
            method: 'PUT',
            body: formData,
    ...
    private static toggleButtonIcon(button: HTMLElement): void {
        const isPublished = button.querySelector('.comment-unpublish-svg');
