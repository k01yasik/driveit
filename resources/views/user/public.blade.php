                ...
                <li class="users-element">
                    ...
                    @if (in_array($profile->user->id, $confirmedFriends, true))
                        <div class="confirmed right">
                            <svg version="1.1" id="checked" xmlns="http://www.w3.org/2000/svg" viewBox="0 0 26 26" xmlns:xlink="http://www.w3.org/1999/xlink" enable-background="new 0 0 26 26">
                    ...
                    @elseif(in_array($profile->user->id, $requestedFriends, true))
                        <div class="waiting right" title="{{ __('Waiting for confirmation.') }}">
                            <svg version="1.1" id="hourglass" xmlns="http://www.w3.org/2000/svg" xmlns:xlink="http://www.w3.org/1999/xlink" viewBox="0 0 60 60" style="enable-background:new 0 0 60 60;" xml:space="preserve">
                                    <g>
                    ...
                    @else
                        <form method="POST" action="{{ route('users.store') }}" class="request-friend right" title="{{__('Send request to add to friends')}}">
                            @csrf
                            <input type="hidden" name="friend" value="{{ $profile->user->id }}">
                            <button>
                                <svg version="1.1" id="plus" xmlns="http://www.w3.org/2000/svg" xmlns:xlink="http://www.w3.org/1999/xlink" viewBox="0 0 533.333 533.333" style="enable-background:new 0 0 533.333 533.333;"
                                     xml:space="preserve">
                                <g>
